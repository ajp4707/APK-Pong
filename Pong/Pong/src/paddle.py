...
class Paddle(pygame.sprite.Sprite):
    ...
    def __init__(self, color, width, height, screenSizeTuple):
        ...
        self.screenW, self.screenH = screenSizeTuple
    ...
    def getservePercent(self):
        return self.rect.y / (self.screenH - self.height) 
        # returns normalized position from middle of the screen

class PaddleLeft(Paddle):
    def __init__(self, color, width, height, screenSizeTuple):
        super().__init__(color, width, height, screenSizeTuple)
        self.rect.x = 0
        self.rect.centery = self.screenH // 2

class PaddleRight(Paddle):
    def __init__(self, color, width, height, screenSizeTuple):
        super().__init__(color, width, height, screenSizeTuple)
        self.rect.x = self.screenW - width
        self.rect.centery = self.screenH // 2